...
function getMessageConfig() {
  const welcome = '👋 Hallo @user\n\n                *W E L C O M E*\n⫹⫺ In @subject\n\n⫹⫺ Read *DESCRIPTION*\n@desc'
  const bye = '👋 Byee @user\n\n                *G O O D B Y E*'
  const spromote = '*@user* Sekarang jadi admin!'
  const sdemote = '*@user* Sekarang bukan lagi admin!'
  const sDesc = 'Deskripsi telah diubah menjadi \n@desc'
  const sSubject = 'Judul grup telah diubah menjadi \n@subject'
  const sIcon = 'Icon grup telah diubah!'
  const sRevoke = 'Link group telah diubah ke \n@revoke'
  const sAnnounceOn = 'Group telah di tutup!\nsekarang hanya admin yang dapat mengirim pesan.'
  const sAnnounceOff = 'Group telah di buka!\nsekarang semua peserta dapat mengirim pesan.'
  const sRestrictOn = 'Edit Info Grup di ubah ke hanya admin!'
  const sRestrictOff = 'Edit Info Grup di ubah ke semua peserta!'
